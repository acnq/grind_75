...
public:
    ...
    void insert(int v) {
        if (root == nullptr)
        {
            ...
            bool isAddLeft = v <= node->val; // 是否将新节点添加到node的左子结点？
...
public:
    ...
    //  1. 令node=根结点，开始搜索
    //  2. 操作如下：
    //      1.若node的左子树的节点数|left| < k - 1, 则第k小的元素一定在node的右子树中
    //        令node=node->right, k = k - |left| - 1
    //      2. 若node的左子树的节点数|left| = k - 1, 则第k小的元素即为node, 返回他即可
    //      3. 若node的左子树节点数|left| > k - 1, 则第k小元素在node的左子树中，node=node->left, 继续搜索

    // III. AVL树
    // 性质
    //  1. 每个结点的左子树和右子树高度最高差1；
    //  2. 每个子数也是平衡二叉树
    //  3. n个节点的平衡二叉搜索树的高度O(logn)
